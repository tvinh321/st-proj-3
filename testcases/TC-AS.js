...
module.exports = async function main(driver) {
    ...
    const assignmentsubmit = await fs.readFileSync('data/assignmentsubmit.txt', 'utf-8');
    ...
    for (let i = 0; i < assignmentsubmitArray.length; i++) {
        ...
        await driver.get('https://hihimoodle.gnomio.com/mod/assign/view.php?id=2&action=removesubmissionconfirm');
        await driver.findElement(By.css(".btn-primary")).click();

        await driver.get('https://hihimoodle.gnomio.com/mod/assign/view.php?id=2&action=editsubmission');
        ...
        for (let j = 0; j < assignmentData.length - 2; j++) {
            ...
            if (assignmentResult === "maxfile" && !(await driver.findElement(By.className("fp-btn-add")).isDisplayed())) {
            ...
            if (assignmentResult === "maxsize") {
                let errorText = await driver.findElement(By.xpath("//div[9]/div[3]/div/div[2]/div/div")).then((element) => {
                ...
                if (errorText === "") {
                    errorText = await driver.findElement(By.xpath("//div[10]/div[3]/div/div[2]/div/div")).then((element) => {
                        return element.getText();
                    }).catch(() => {
                        return "";
                    });
                ...
                if (errorText.includes("The maximum size you can upload is 10 MB.")) {
                    console.log(`TC-AS-${i + 1}: Passed`);
                    maxSizeError = true;
                    break;
                }
                else {
                    console.log(`TC-AS-${i + 1}: Failed`);
                }
            }
        }
        ...
        if (assignmentResult === "success") {
            ...
            for (let j = 0; j < assignmentData.length - 2; j++) {
                const assignment = assignmentData[j];
                driver.findElement(By.xpath(`//a[contains(.,'${assignment}')]`)).isDisplayed().then(() => {
                    result = true;
                }
                ).catch(() => {
                    result = false;
            ...
            const errorText = String(await driver.findElement(By.xpath("//section/div[2]/div")).getText());
